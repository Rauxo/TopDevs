        ...
        <div className="p-8 pb-4">
          ...
          <div className="relative group">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-4 bg-slate-100/50 border border-transparent rounded-[24px] text-sm font-bold outline-none focus:bg-white focus:border-emerald-500/30 transition-all"
            />
          </div>
        </div>
                 ...
                 {pendingRequests.map(conv => (
                   ...
                   >
                     <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white shadow-lg shadow-amber-500/20">
                          <User size={24} />
                        </div>
             ...
             <div className="space-y-3">
               {activeChats.length > 0 ? activeChats.map(conv => (
                 <div 
                  key={conv._id} 
                  onClick={() => setActiveConv(conv)}
                  className={`p-4 rounded-[28px] cursor-pointer transition-all border-2 ${activeConv?._id === conv._id ? "bg-emerald-50 border-emerald-200 shadow-lg shadow-emerald-200/20" : "bg-white/50 border-transparent hover:bg-white hover:shadow-md"}`}
                 >
                   <div className="flex items-center gap-4">
                      {conv.otherParticipantType === "User" ? (
                        ...
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-emerald-400 to-cyan-500 flex items-center justify-center text-white shadow-lg shadow-emerald-500/20">
                          <Building2 size={24} />
                        </div>
                      )}
                      <div className="flex-1 overflow-hidden">
        ...
        {activeConv ? (
               ...
               <div className="flex items-center gap-4">
                  ...
                  <div className="relative">
                    <div className="w-14 h-14 rounded-2xl bg-slate-100 flex items-center justify-center text-slate-600 border-2 border-white shadow-sm overflow-hidden">
                      {activeConv.otherParticipantType === "User" ? (
                        ...
                        <div className="w-full h-full bg-emerald-500 flex items-center justify-center text-white"><Building2 size={28} /></div>
                      )}
                    </div>
                  ...
                  <div>
                    ...
                    <button 
                      ...
                      className="text-[10px] text-emerald-600 font-black uppercase tracking-widest flex items-center gap-1.5 hover:text-emerald-700 transition-all border-none bg-transparent cursor-pointer group"
                    >
               ...
               <div className="flex items-center gap-3">
                 {type === "company" && activeConv.status === "pending" && (
                   <button 
                    ...
                    className="px-6 py-3 bg-emerald-600 text-white text-xs font-black uppercase tracking-widest rounded-2xl hover:bg-emerald-700 transition-all flex items-center gap-2 border-none cursor-pointer shadow-xl shadow-emerald-500/20"
                   >
                ...
                >
                  <div className={`max-w-[75%] group`}>
                    ...
                    <div className={`flex items-center gap-2 mt-2 px-2 ${msg.senderId === currentId ? "justify-end" : "justify-start"}`}>
                       ...
                       {msg.senderId === currentId && <div className="w-1 h-1 bg-emerald-500 rounded-full"></div>}
                    </div>
                  </div>
                </div>
              ))}
            ...
            {activeConv.status === "accepted" || (activeConv.status === "pending" && activeConv.initiatedBy.type === (type === "user" ? "User" : "Company")) ? (
              <form onSubmit={handleSendMessage} className="p-8 border-t border-slate-100 bg-white/80 backdrop-blur-xl">
                <div className="relative flex items-center gap-4">
                  ...
                  <button 
                    type="submit"
                    className="p-5 bg-slate-900 text-white rounded-[24px] hover:bg-emerald-600 transition-all shadow-xl shadow-slate-900/10 border-none cursor-pointer flex items-center justify-center group active:scale-95"
                  >
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-300 p-12 text-center bg-slate-50/30">
             <div className="w-32 h-32 bg-white rounded-[48px] shadow-2xl flex items-center justify-center mb-10 border border-white animate-bounce-slow">
               <MessageSquare size={64} className="text-emerald-500/30" />
             </div>
