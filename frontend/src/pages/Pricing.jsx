        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {plans.map((plan) => (
              <div 
                key={plan._id}
                className="bg-white rounded-[40px] shadow-xl border border-slate-100 p-10 flex flex-col hover:-translate-y-2 transition-all duration-500 hover:shadow-2xl hover:border-blue-200 group"
              >
                <div className="mb-8">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 text-white shadow-lg ${
                    ...
                    plan.name === 'Standard' ? 'bg-slate-500 shadow-slate-500/20' : 
                ...
                <button 
                  ...
                  className="w-full py-5 bg-slate-900 text-white font-black uppercase tracking-widest rounded-2xl hover:bg-blue-600 transition-all shadow-xl shadow-slate-900/10 border-none cursor-pointer group-hover:scale-105 active:scale-95"
                >
                  Get Started
                </button>
              </div>
